...
export default async function RealEstate() {
  const [listings, featured] = await Promise.all([
    fetchPortalListings('Real Estate', 'Detailed-RealEstate'),
    fetchFeaturedCategoryListings('Real Estate', 'Detailed-RealEstate'),
  ])

  return (
    <>
      {featured.length > 0
        ? <FeaturedListingsCarousel listings={featured} categoryLabel="Real Estate" />
        : (
          <>
